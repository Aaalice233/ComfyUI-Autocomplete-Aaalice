...
- 現行の ComfyUI フロントエンドに合わせた継続的なメンテナンス
- **Nodes 2.0** で描画されるテキスト入力への対応
- **サブグラフノード**に昇格されたテキスト入力への対応、および元の内部ノードとウィジェットの特定
- オートコンプリートと関連タグの連携および挿入形式を統一：入力途中のタグでは補完候補を再表示し、補完確定後は関連タグを即座に表示します。末尾のカンマから直前のタグを認識し、関連タグ挿入時は既存の区切りを再利用して空タグや重複カンマを作りません
- カテゴリ別の投稿数条件で Danbooru の最新タグを補完し、再開可能な DeepSeek 翻訳とローカルキャッシュで翻訳名を追加します
- LoRA Manager のローカルタグ、LoRA、Embedding、Wildcard API から候補を補完し、LoRA Manager 自身の入力欄では重複して起動しない互換レイヤーを追加
- 一般、アーティスト、作品、キャラクター、メタ、モデルなどのカテゴリ別アイコンとローカライズされたホバー説明を追加
- Danbooru、e621、LoRA Manager の候補をソース別に分けず、関連度で統一的に並べ替え
- 簡体字中国語ドキュメントの追加とローカライズの継続的なメンテナンス
- オートコンプリートと関連タグの見出し、読み込み・空状態、操作ボタン、Wiki リンク、詳細ツールチップを英語・簡体字中国語・繁体字中国語・日本語に対応
...
- **:arrows_counterclockwise:ライブタグ補完**: 基本 Danbooru CSV にないタグを取得し、DeepSeek で翻訳可能
...
- 候補ごとにカテゴリ別アイコンを表示し、カーソルを合わせるとカテゴリとデータソースを確認できます
- 全ソースを「タグ完全一致、タグ前方一致、エイリアス完全一致、タグ部分一致、エイリアス部分一致」の順で統一し、その後に各ソース内で正規化した人気度を比較します
...
- **タグカテゴリのアイコン位置**: カテゴリアイコンの表示位置を指定します。カーソルを合わせるとカテゴリとデータソースが表示され、「hidden」で非表示にできます
...
**Autocomplete Plus → ライブタグ → Danbooru ライブタグを管理** から管理画面を開きます。この機能は Hugging Face の基本 CSV を変更しません。

1. Danbooru の各カテゴリに「取得しない」「すべて取得」「最低投稿数」を個別に設定します。
2. 必要に応じて Danbooru のログイン名と API Key を設定し、「タグをスキャン」を実行します。`danbooru_tags.csv` にないタグだけが `data/danbooru_tags_live.csv` に保存されます。
3. 新規候補数と推定リクエスト数を確認してから DeepSeek 翻訳を開始します。翻訳言語は現在の ComfyUI 表示言語に従い、英語では翻訳を実行しません。
4. CSV 更新後にページを再読み込みし、オートコンプリートのインデックスを再構築します。

DeepSeek のバッチ数、最大同時実行数、再試行回数、モデル、system prompt を設定できます。成功した翻訳は「タグ + 言語」単位で SQLite にキャッシュされ、以後は未翻訳または明示的に再試行したタグだけが送信されます。中断またはキャンセルしても完了済みの結果は保持されます。

設定とキャッシュは ComfyUI ユーザーディレクトリの `autocomplete-plus/` に保存されます。バックエンドは API Key の平文を返さず、ログにも出力しません。本機能はローカルまたは非公開環境向けです。ComfyUI サーバーへアクセスできるユーザーは翻訳を開始して API 料金を発生させる可能性があります。
