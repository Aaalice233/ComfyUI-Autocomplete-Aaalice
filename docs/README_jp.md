...
- Danbooru スキャンは設定可能な適応型同時実行数（1～16、既定値 8）で 16 個の ID 範囲を処理し、スキャンは処理件数のみ、翻訳は正確な進捗を表示します
...
3. 新規候補数、基本 CSV の翻訳不足数、推定リクエスト数を確認してから DeepSeek 翻訳を開始します。翻訳言語は現在の ComfyUI 表示言語に従い、英語では翻訳を実行しません。
4. CSV 更新後にページを再読み込みし、オートコンプリートのインデックスを再構築します。

DeepSeek は新規スキャンタグに加え、基本 CSV の別名に現在の表示言語がないタグも処理します。バッチ数、最大同時実行数、再試行回数、モデル、system prompt を設定できます。成功した翻訳は「タグ + 言語」単位で SQLite にキャッシュされ、以後は未翻訳または明示的に再試行したタグだけが送信されます。生成される追加 CSV は基本タグのカテゴリ、投稿数、既存の別名を保持して翻訳だけを追加し、元の基本 CSV は変更しません。中断またはキャンセルしても完了済みの結果は保持されます。
...
Danbooru はこのフィルター付きスキャンの安定した総件数を低コストで返せないため、スキャン中は誤解を招くパーセント表示をやめ、スキャン済みタグ数を表示します。各カテゴリは重複しない 16 個のタグ ID 範囲に分割され、同時実行数は 1～16（既定値 8）で設定できます。HTTP 429 が返ると現在の同時実行数を自動的に半減し、成功が続くと段階的に回復します。翻訳は開始前に総件数が確定するため、通常の進捗バーを維持します。
