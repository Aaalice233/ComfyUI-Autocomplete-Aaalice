...
- ライブタグのネットワーク、認証、CSV、翻訳エラーを ComfyUI の表示言語で案内し、未知の診断情報も保持します
- Danbooru 同期では Cloudflare と互換性のある製品識別子を使用し、`ComfyUI` を含む User-Agent が一部のエッジルールに拒否される問題を回避します
- Danbooru スキャンは設定可能な適応型同時実行数（1～16、既定値 8）で 16 個の再開可能な ID 範囲を処理し、翻訳はキャッシュ、成功、失敗、保留をセグメント表示します
...
- **:arrows_counterclockwise:ライブタグソース**: Danbooru のフィルター済み完全スナップショットを取得し、Hugging Face タグソースを置き換えて DeepSeek で翻訳可能
...
**Autocomplete Plus → ライブタグ → Danbooru ライブタグを管理** から管理画面を開きます。Danbooru スナップショット作成後は、それだけを Danbooru タグソースとして使用し、LoRA Manager 互換補完は維持します。Hugging Face ファイルはフォールバックとしてローカルに残ります。

1. Danbooru の各カテゴリに「取得しない」「すべて取得」「最低投稿数」を個別に設定します。
2. 必要に応じて Danbooru のログイン名と API Key を設定し、「タグをスキャン」を実行します。フィルター済みの完全スナップショットが `data/danbooru_tags_live.csv` に保存され、Danbooru オートコンプリートのデータソースになります。
   認証情報には「Danbooru を開く」ショートカットもあり、公式サイトを新しいタブで開けます。
3. 現在のタグ総数、翻訳不足数、推定リクエスト数を確認してから DeepSeek 翻訳を開始します。翻訳言語は現在の ComfyUI 表示言語に従い、英語では翻訳を実行しません。
4. CSV 更新後にページを再読み込みし、オートコンプリートのインデックスを再構築します。

DeepSeek は「既存翻訳をスキップ」と「すべて再翻訳」を選択できます。成功した翻訳は「タグ + 言語」単位でローカル SQLite 辞書に保存され、生成 CSV に統合されます。管理画面はサイドバーでタスク概要、タグ取得、翻訳設定、タグ統計、ローカル辞書、認証設定に分かれます。フッターには現在のページに必要な主要操作だけを表示し、再試行と全再翻訳は翻訳ページ内に配置します。カテゴリとソース別の件数やタグ一覧も確認できます。スキャンは実行中に CSV とチェックポイントへ保存され、スキャンと翻訳は異常終了、失敗、手動キャンセル後も再開できます。翻訳進捗は辞書ヒット、完了、失敗、保留を色分けしたセグメントバーで表示します。

Danbooru スキャンには専用の `Autocomplete-Plus` User-Agent を使用します。一部の Cloudflare エッジでは、同じ `aiohttp` とネットワーク出口でも、製品識別子に `ComfyUI` が含まれるリクエストだけが拒否されます。

Danbooru はこのフィルター付きスキャンの安定した総件数を低コストで返せないため、スキャン中は件数、カテゴリ分布、完了範囲、処理速度を表示し、誤解を招くパーセント表示を行いません。各カテゴリは重複しない 16 個のタグ ID 範囲に分割され、同時実行数は 1～16（既定値 8）で設定できます。HTTP 429 が返ると現在の同時実行数を自動的に半減し、成功が続くと段階的に回復します。

入力補完と関連タグ画面では、現在の ComfyUI 表示言語に合う別名だけを表示します。表示のみの絞り込みで、すべての別名は検索対象のままです。
