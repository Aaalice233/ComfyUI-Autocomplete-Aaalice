...
- 同梱の Hugging Face CSV を即座に表示し、LoRA Manager と Danbooru はバックグラウンドで並行して補足。同名タグは `CSV > LoRA Manager > Danbooru`、別タグは一致精度、人気度、取得元優先度の順で並べます
...
- 非同期結果で再並べ替えしても、タグ名で現在の選択を維持して誤選択を防止
...
- 簡体字中国語では ffdkj 辞書を優先し、次にキャッシュ済みまたは新規の DeepSeek 翻訳を使用。DeepSeek は辞書にない項目だけを処理し、繁体字中国語と日本語は従来どおり DeepSeek を使用
- 実際の選択リストによるモデル検出、稼働確認、保存済み API Key のオンデマンド表示、任意の思考強度に対応
- **オンラインサービス**を Danbooru API、中国語辞書、DeepSeek LLM の 3 ページ管理画面へ再構成。デスクトップでは固定サイドバー、狭い画面では上部タブを使用
- 辞書の確認、更新、修復は DeepSeek 設定の保存とは独立して即時実行し、各サービスに状態、進捗、エラー、キャッシュ情報を表示
...
オートコンプリートは安全上限付きの完全なローカル結果スナップショットを即座に計算し、LoRA Manager と最大 200 件の Danbooru スナップショットを並行取得して統合します。同名タグは `CSV > LoRA Manager > Danbooru` を厳守し、別タグは一致精度、人気度、取得元優先度の順で並べます。非同期の再並べ替え後もタグ名で現在の選択を復元します。2～3 文字は前方一致、4 文字以上は部分一致を使い、スクロール処理からデータ取得は行いません。
...
成功した Danbooru 補完ページと関連タグスナップショットは `completion_cache.sqlite3` に保存されます。補完、関連タグ、ヘルスチェックはリクエスト統合、キャッシュ、匿名読み取りレート制限を共有し、クライアント側は毎秒 8 リクエスト以下です。Danbooru のアカウント、パスワード、API Key は不要です。管理画面でキャッシュ件数と容量を確認・消去できます。
...
簡体字中国語の表示と中国語検索は、まず [`ffdkj-Danbooru_Tag-Chinese-English-Translation-Table`](https://github.com/ffdkj/ffdkj-Danbooru_Tag-Chinese-English-Translation-Table) の辞書を使用し、次にキャッシュ済みおよび新規 DeepSeek 翻訳を使用します。このモードでは Hugging Face CSV に混在する CJK 別名を無視します。アーティスト訳は辞書に原文と異なる非空値がある場合だけ表示し、DeepSeek には送りません。繁体字中国語と日本語は DeepSeek、英語は翻訳なしです。

ffdkj 上流リポジトリには現在明確な LICENSE がないため、このプラグインはデータベースを再配布しません。簡体字中国語の初回利用時に約 29.3 MB を ComfyUI ユーザーディレクトリの `autocomplete-plus/chinese-dictionary/` へバックグラウンドでダウンロードします。ダウンロード中も英語補完は利用でき、失敗した場合のみ DeepSeek が補完します。以後の確認と更新は手動です。Git blob SHA、SQLite ヘッダーとスキーマ、`PRAGMA quick_check`、有効行数を検証してから原子的に置換するため、更新失敗時も旧 DB を保持します。中国語タグ翻訳を維持している ffdkj と貢献者に感謝します。データの管理と帰属は上流プロジェクトにあります。

管理画面は **Danbooru API**、**中国語辞書**、**DeepSeek LLM** の 3 ページです。辞書の確認、インストール/更新、修復は設定保存とは独立して即時実行します。DeepSeek ページには `/models` からのモデル取得、API Key 表示、モデルテスト、思考強度、並行数、バッチ、再試行、タイムアウト、システムプロンプトがあります。中国語辞書は簡体字中国語だけに適用され、`translations.sqlite3` は独立した LLM キャッシュとして残ります。
