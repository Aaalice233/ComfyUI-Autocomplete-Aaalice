...
- 上流版で大規模な CSV とモデルのインデックス完了を起動時に待つため、ComfyUI の全体ローディング画面が長時間消えない問題を修正。拡張機能の登録は同期で完了し、インデックスはバックグラウンドで構築され、準備中も安全なフォールバックで補完を利用でき、失敗したローカルデータは再試行できます。
...
他の ComfyUI 拡張機能は、独自の入力欄に `data-autocomplete-plus` 属性を付けるだけで補完を有効にできます。要素は自動的に検出され、ノードのテキストエリアと同じタグ補完・中国語補完・関連タグが利用できます。候補パネルが開いている間、要素には `data-autocomplete-plus-open` 属性が付くので、ホスト側の UI は Enter、Esc、矢印キーをパネルに譲ることができます。
