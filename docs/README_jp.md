...
- 補完と関連タグの末尾に専用の取得元列を設け、`CSV`、`LM`、`API` バッジが英語タグを圧迫しないようにします。同じタグは全取得元を保持し、ホバーで詳細を確認可能
...
- 上位結果を先に翻訳し、その後も未翻訳行を制御されたバッチで順次補完。完了したバッチは直ちに一覧へストリーム反映し、処理中の行だけにアニメーション付き 3 点インジケーターを表示
