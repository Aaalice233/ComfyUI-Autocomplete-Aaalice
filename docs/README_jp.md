...
- 固定数の上位結果を制御された並行バッチで先行翻訳し、スクロール処理から完全に分離。DeepSeek の結果は永続的で検索可能な辞書へ保存
- モデル検出、稼働確認、任意の思考強度に対応
...
- 見出し、読み込み・空状態、操作ボタン、Wiki リンク、取得元バッジ、ツールチップを英語・簡体字中国語・繁体字中国語・日本語に対応
...
**Autocomplete Plus → オンラインサービス → オンライン補完と翻訳** で DeepSeek を設定します。クエリが安定すると補完は先頭 200 件、関連タグは先頭 300 件を一度だけ事前翻訳します。候補は固定サイズに分割し、フロントエンドから最大 3 リクエストを並行実行し、バックエンドでも重複タグを除外します。翻訳処理はスクロールから完全に分離され、スクロールによる再リクエストは発生しません。キャッシュ済み訳は即座に表示され、新しい訳は現在の仮想ウィンドウだけに反映されます。LoRA、Embedding、Wildcard は翻訳対象外です。成功した翻訳は `translations.sqlite3` に保存され、一致するすべての booru 検索インデックスにも追加されます。

設定画面には「Danbooru API 補足を有効化」と「自動翻訳を有効化」の独立スイッチがあります。Danbooru を無効にすると補完と関連タグの API リクエストを停止し、永続キャッシュや翻訳辞書にある API 専用候補も一覧から除外します。翻訳を無効にすると辞書の読み込みと自動翻訳リクエストを停止しますが、翻訳 DB は削除せず再有効化後に再利用できます。オンラインソースの確認、モデル一覧と稼働確認、Danbooru 結果キャッシュの統計・消去、翻訳の詳細設定も同じ画面で操作できます。
