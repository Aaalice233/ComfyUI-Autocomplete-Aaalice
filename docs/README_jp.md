...
- Hugging Face を常時ローカル基盤とし、LoRA Manager があれば候補を統合し、不足分だけを Danbooru API で静かに補う 3 段階のタグ取得
- 表示中の候補だけを DeepSeek でオンデマンド翻訳し、検索可能なローカル辞書へ保存。モデル選択、モデル一覧取得、稼働確認、思考強度に対応し、思考は既定でオフ
- 別名の表示を現在の ComfyUI 言語で絞り込み、完全な別名セットは検索に利用できます
- Danbooru と翻訳の失敗は入力を妨げず、直近の診断だけを簡潔な設定画面に表示
- フレーム単位の検索統合、候補プールの上限、CSV の分割解析、翻訳インデックスと SQLite 処理の遅延実行、関連タグ計算のキャッシュにより入力とパネル操作の停止を抑制
- LoRA Manager のローカルタグ、LoRA、Embedding、Wildcard API から候補を補完し、LoRA Manager 自身の入力欄では重複して起動しない互換レイヤーを追加
- 一般、アーティスト、作品、キャラクター、メタ、モデルなどのカテゴリ別 Emoji とローカライズされたホバー説明を追加
- Danbooru、e621、LoRA Manager の候補をソース別に分けず、人気度優先で統一的に並べ替え
...
- **:arrows_counterclockwise:オンラインタグ補完**: ローカル結果が不足した場合だけ Danbooru から静かに補完し、別の大型タグ DB は作成しません
...
- **1ページの補完候補数**: 1回のスクロールで読み込む補完候補数（既定値：15）。末尾までスクロールすると次ページを読み込み、結果総数は制限しません
...
- **1ページの共起タグ数**: 1回のスクロールで読み込む共起タグ数（既定値：15）。末尾までスクロールすると次ページを読み込みます
...
オートコンプリートは常にダウンロード済み Hugging Face CSV を先に検索し、LoRA Manager が利用可能ならローカル候補を統合します。最初のページが不足する場合は Danbooru が補い、ローカル候補だけで最初のページが埋まっていても、下へスクロールすると Danbooru の結果をページ単位で取得して統合します。そのためページ件数は1回の読み込み量だけを制御し、結果総数は制限しません。投稿数 0 の空タグ、非推奨タグ、未対応カテゴリに加え、旧翻訳辞書に残った投稿数 0 のオンラインタグもバックエンドとフロントエンドの両方で除外されるため、未使用の記号バリエーションが一覧を埋めることはありません。長いタグ名は省略表示され、翻訳と人気度の列を画面外へ押し出しません。失敗、タイムアウト、レート制限は表示されず、既存候補も消えません。生の結果は現在のブラウザーセッションだけで短期キャッシュされ、ローカルタグスナップショットにはなりません。

Danbooru API だけから補われた候補には、タグ名の後ろに低コントラストの小さな `API` チップを表示します。チップにカーソルを合わせるとオンライン補完元を確認でき、意味を明確にしつつ候補一覧を目立って乱しません。

**Autocomplete Plus → オンラインサービス → オンライン補完と翻訳** で DeepSeek を設定します。ローカルまたは補足候補の各バッチが表示された時点で、通常の Danbooru および e621 タグのバックグラウンド翻訳を開始し、LoRA Manager や Danbooru の応答完了を待ちません。キャッシュ済み訳は即座に表示され、新しい訳は同じ行へ後から追加されます。LoRA、Embedding、Wildcard は翻訳対象外です。オンライン翻訳またはキャッシュが利用できる場合は Hugging Face CSV の同一言語の旧訳を置き換え、オンライン翻訳が利用できない場合は CSV の訳をフォールバックとして残します。成功した翻訳は一度だけ `translations.sqlite3` に保存され、一致するすべての booru 検索インデックスにも追加されます。Key を削除した場合やサービスがオフラインの場合も既存辞書は利用できます。

設定画面ではすべてのオンラインデータソースを明示的に確認し、現在の Key でモデル一覧を取得して、選択したモデルへ小さな稼働確認リクエストを送信できます。受動的なデータソースは最初のリクエスト前に「最初の補完待ち」または「フォールバック待ち」と表示され、確認操作のたびに状態カードが即座に更新されます。思考強度は「オフ」「高」「最大」から選べ、通常のタグ翻訳には遅い推論が不要なため既定値は「オフ」です。同時実行数、バッチ、再試行、タイムアウト、システムプロンプトは既定で折りたたまれた詳細設定に残ります。
