...
- 以受控并发批次预取固定数量的前排译文，并与滚动彻底解耦；DeepSeek 译文会形成持久、可搜索的本地词典。
- 支持模型发现、测活和可选思考强度。
...
- 标题、加载与空状态、操作按钮、Wiki 链接、来源徽章及提示均适配英文、简体中文、繁体中文和日文。
...
在 **Autocomplete Plus → 在线服务 → 在线补全与翻译** 中配置 DeepSeek。每次查询稳定后，补全会一次预取前 200 条、共现列表会一次预取前 300 条译文；候选按固定大小分批，前端最多同时发送 3 个翻译请求，后端继续去重重叠词条。翻译任务与滚动完全解耦，滚动不会再次请求翻译。缓存译文立即显示，新译文只更新当前虚拟窗口。LoRA、Embedding 和 Wildcard 不进入翻译流程。在线翻译或缓存译文有效时，会替换 Hugging Face CSV 中同语言的旧译名；在线翻译不可用时仍保留 CSV 译名作为兜底。成功译文写入 `translations.sqlite3` 并加入所有匹配的 booru 搜索索引；删除 Key 或服务离线后仍可继续使用已有词典。

面板提供独立的“启用 Danbooru API 补充”和“启用自动翻译”开关。关闭 Danbooru 后，补全与共现都不会再请求接口，持久缓存或翻译词典中仅由 API 补充的候选也不会进入列表；关闭翻译后停止加载翻译词典和自动请求译文，但不会删除翻译数据库，重新开启即可继续使用。面板还支持检测在线数据源、拉取和测试模型、查看或清空 Danbooru 结果缓存，并保留并发、批量、重试、超时和系统提示词等高级设置。
