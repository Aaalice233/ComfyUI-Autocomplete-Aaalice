...
- 优先翻译前排结果，再以受控批次逐步补齐所有仍无译文的条目，全程不依赖滚动；每批完成后立即流式更新列表，只有仍在请求的条目保留动态三点指示器。拒绝原样返回或缺少目标语言文字的伪译文；画师标签不翻译，直接显示原始名称。
