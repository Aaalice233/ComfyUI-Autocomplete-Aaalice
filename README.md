...
- Translates the leading results first, then progressively backfills every still-untranslated row in bounded batches without depending on scrolling. Each completed batch streams into the list immediately, while animated three-dot indicators remain only on rows still in flight.
