...
- Translates the leading results first, then progressively backfills every still-untranslated row in bounded batches without depending on scrolling. Each completed batch streams into the list immediately, while animated three-dot indicators remain only on rows still in flight. Identity or wrong-script outputs are rejected; artist tags skip translation and display their original names.
...
- Rebuilds the online-services panel around compact service switches, at-a-glance health indicators, restrained cache controls, a focused DeepSeek form, an API-key visibility toggle, summarized advanced settings, and a single clear save action, finished with borderless layered surfaces and soft edge shadows.
...
Open **Autocomplete Plus → Online Services → Online completion and translation** to configure DeepSeek. Once a query stabilizes, autocomplete places its first 200 candidates and related tags place their first 300 candidates at the front of one translation job, followed by every row that still has no localized alias. The frontend submits that complete job once; the backend owns batching and enforces the configured `concurrency` as one global DeepSeek limit across all jobs, while SingleFlight continues to merge duplicate tags. This avoids a hidden browser-side concurrency cap and makes both `batch_size` and `concurrency` effective; the default batch size is 20. Translation is fully detached from scrolling. Completed backend batches stream into the current virtual window immediately instead of waiting for the slowest batch or a scroll-triggered redraw; only unresolved rows retain the animated three-dot indicator. Cached aliases render immediately. Outputs identical to the source tag or missing the target language's writing system are rejected and cannot replace a valid CSV alias; stale invalid cache rows are removed when the store opens. Artist tags are not sent to DeepSeek and use their original tag names in the alias column. LoRA, Embedding, and Wildcard candidates are also excluded. When an online or cached translation is available, it replaces the same-language alias bundled in the Hugging Face CSV; the CSV alias remains the fallback when online translation is unavailable. Successful results are stored once in `translations.sqlite3`, added to every matching booru search index, and reused on later inputs.

The panel provides independent **Enable Danbooru API supplementation** and **Enable automatic translation** switches. Disabling Danbooru stops both completion and related-tag API requests and prevents persisted API-only candidates from entering lists; disabling translation stops catalog loading and automatic resolve requests while preserving the translation database for later re-enabling. The panel can explicitly check online sources, fetch every available model into a visible selection list, test the selected model, reveal the saved API key on demand, inspect or clear Danbooru-result-cache statistics, and configure advanced translation controls.
